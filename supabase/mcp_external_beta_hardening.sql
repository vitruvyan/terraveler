...
-- The advisory lock closes the absent-row race: SELECT ... FOR UPDATE cannot
-- serialize two first writers when the row does not exist yet.
...
begin
  perform pg_advisory_xact_lock(hashtextextended(
    'mcp-idem:' || p_agent_account_id::text || ':' || p_action || ':' || p_key_hash, 0));
