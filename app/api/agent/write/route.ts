...
async function optionalRpc(name: string, args: Record<string, unknown>) {
  try {
    ...
    // Production external beta must never silently downgrade from an atomic RPC
    // to a multi-request compatibility path. The fallback exists only for
    // controlled migration/testing when DB guards are explicitly not required.
...
export async function POST(req: Request) {
  ...
  try {
    ...
    await securityAudit({ source, action: name, outcome: status < 400 ? "accepted" : "rejected", status,
      reason: status < 400 ? null : text.slice(0, 500), agentId: bearer.agent_id,
    ...
    await securityAudit({ source, action: name, outcome: "failed", status: 500,
      reason: "protected mutation failed", agentId: bearer.agent_id,
      agentAccountId: bearer.agent_account_id, connectionId: bearer.connection_id, clientId: bearer.client_id });
