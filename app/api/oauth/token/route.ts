...
function fail(error: string, description: string, status = 400, headers?: Record<string, string>) {
...
async function readParams(req: Request): Promise<Record<string, string>> {
  ...
  const parsed = await readLimitedText(req, ENROLLMENT_BODY_LIMIT);
  if (!parsed.ok) throw Object.assign(new Error(parsed.error), { status: parsed.status });
  return Object.fromEntries(new URLSearchParams(parsed.value));
}
...
async function authorizationCode(p: Record<string, string>) {
  ...
  if (!row) {
    ...
    const bindingsHold = prior.client_id === client_id &&
      redirectAllowed([prior.redirect_uri], redirect_uri) && pkceMatches(code_verifier, prior.code_challenge);
    if (bindingsHold) {
      await detonate(prior.connection_id,
        `authorization code redeemed twice by a holder of the PKCE verifier (client ${client_id}); connection revoked`);
      return fail("invalid_grant",
        "this code has already been redeemed. Two holders of the verifier means one is not you, so the connection has been revoked — authorise again.");
    }
  ...
  if (row.client_id !== client_id) return fail("invalid_grant", "this code was issued to a different client");
  ...
  if (!pkceMatches(code_verifier, row.code_challenge)) return fail("invalid_grant", "PKCE verification failed");
  if (!resourceOk(p.resource, row.resource)) return fail("invalid_target", "this code was issued for a different resource");

  const tokens = await issueTokens(row.connection_id, row.scopes, row.resource ?? MCP_RESOURCE);
  return NextResponse.json({ ...tokens, token_type: "Bearer", scope: (row.scopes ?? []).join(" ") }, { headers: noStore });
}
...
async function refresh(p: Record<string, string>) {
  ...
  if (!tok) {
    ...
    if (prior.rotated_to) {
      const rotatedRecently = Date.now() - new Date(prior.revoked_at ?? prior.created_at).getTime() < REUSE_GRACE_MS;
      if (rotatedRecently) {
        const family = await sb("GET", `oauth_tokens?id=eq.${prior.rotated_to}&select=scopes,resource,connection_id`);
        if (family?.[0]) return fail("invalid_grant",
          "this token was just rotated. Use the refresh token from that response — it was issued seconds ago and is still current.");
      }
      await detonate(prior.connection_id,
        `rotated refresh token presented again after the reuse window (client ${client_id ?? "unnamed"}); connection revoked`);
      return fail("invalid_grant",
        "this refresh token was already exchanged. Two holders means one of them is not you, so every token on this connection has been revoked. Authorise again.");
    }
    return fail("invalid_grant", "this token has been revoked");
  }

  if (new Date(tok.expires_at).getTime() < Date.now()) return fail("invalid_grant", "refresh token expired — authorise again");
  ...
  const fresh = await sb("GET", `oauth_tokens?token_hash=eq.${sha256(tokens.refresh_token)}&select=id`);
...
async function clientCredentials(p: Record<string, string>) {
  ...
  if (!client?.client_secret_hash)
    return fail("invalid_client",
      "unknown client, or a client registered for the interactive flow. Register with grant_types: [\"client_credentials\"] to self-enrol an agent.", 401);
  ...
  if (!agentAccountId) {
    ...
    await sb("PATCH", `oauth_clients?client_id=eq.${encodeURIComponent(client_id)}`, { agent_account_id: agent.id });
  } else if (!client.agent_account_id) {
    await sb("PATCH", `oauth_clients?client_id=eq.${encodeURIComponent(client_id)}`, { agent_account_id: agentAccountId });
  }
  ...
  if (existing?.revoked_at)
    return fail("invalid_client",
      "this runtime connection has been revoked. The agent identity and its standing still exist, but this credential may no longer act.", 403);

  if (existing) {
    const merged = [...new Set([...(existing.scopes ?? []), ...scopes])];
    await sb("PATCH", `agent_connections?id=eq.${existing.id}`, {
      scopes: merged, agent_account_id: agent.id, contributor_id: agent.contributor_id,
    });
  } else {
  ...
  return NextResponse.json({
    ...
    scope: scopes.join(" "), agent_id: agent.public_id,
  }, { headers: noStore });
}
