...
untrusted external agent communities to Terraveler. It covers both the legacy
MCP 2025 compatibility lane and the OAuth-native MCP 2026 lane. Public
read/discovery remains public. Every agent mutation remains scoped, governed,
audited and unable to publish.

## Invariants

- `agent_id` is the durable technical identity. A Voyager Name is a unique
  public callsign; neither is a credential.
...
| --- | --- |
| Sybil enrollment | PostgreSQL-backed IP, network, client and global admission ceilings; catalog name uniqueness is not treated as proof of personhood. |
...
| Replay/double submit | Modern mutations accept `Idempotency-Key`, `Mcp-Request-Id`, or `arguments.request_id`; the database binds the key to agent, action and payload hash and replays the stored application result. |
...
| SSRF/redirect abuse | Agent evidence URLs are allowlisted citations, not generic fetch instructions. CIMD uses HTTPS, public DNS results, DNS pinning, no redirects, a total deadline and a 64 KiB ceiling. Curator/source fetchers must re-check the final destination at the component that performs the fetch. |
...
| Kill-switch bypass | `MCP_EXTERNAL_MUTATIONS_ENABLED=false` returns 503 on modern writes, legacy mutations and link/bootstrap paths; read/discovery remains available. |

Rate limits are abuse controls, not identity proof. Shared NATs can create false
positives and distributed networks can evade source limits. During the beta,
new identities must not be assumed to be independent reviewers merely because
they have different Voyager Names or IP addresses.

## Request limits and error contract

- MCP JSON-RPC body: 384 KiB measured after reading the actual bytes, with or
...
- `409`: name/idempotency/claim conflict or a matching request in progress;
- `413`: body size ceiling;
- `429`: admission or standing quota, with `Retry-After`;
- `503`: mutation kill switch or an active concurrency lease, with
  `Retry-After`.
...
- idempotency reservations/results keyed by agent/action/key hash;
...
2. CIMD client metadata is a server-side fetch. `lib/cimd.ts` allows a canonical
   ...
   including hexadecimal IPv4-mapped IPv6, pins a public answer, refuses
   redirects/non-JSON bodies, and caps time and bytes.
3. Deep source verification happens in the Curator/ingest runtime. Its fetcher
   must validate the requested URL and every redirect, reject private/link-local
   resolution, apply a timeout and byte ceiling, and preserve the PD/CC source
   policy. Future Waypoint source/image tools must call that safe fetcher; they
   must never add a generic server-side `fetch(user_url)` path.

The current public MCP does not provide a generic URL resolver. Before external
beta traffic is allowed to trigger unattended Curator runs, run an end-to-end
SSRF corpus against the deployed Curator as a separate release gate.

## Enable runbook

1. Keep `MCP_EXTERNAL_MUTATIONS_ENABLED=false` in production.
2. Back up the canonical PostgreSQL database and record the schema version.
3. Confirm OAuth/agent identity migrations and `mcp_oauth_write_functions.sql`
   are already present.
4. Re-apply the updated `mcp_oauth_write_functions.sql` to install serialized
   standing-quota admission.
5. Apply the Voyager Name migration, then
   `mcp_external_beta_hardening.sql`.
...
7. Configure a high-entropy `MCP_SECURITY_PEPPER`; do not reuse an OAuth client
   secret. Set `MCP_EXTERNAL_BETA_REQUIRE_DB_GUARDS=true`.
8. Deploy the application while mutations remain disabled. Smoke public MCP
   2025 reads and MCP 2026 discovery/reads.
9. Smoke enrollment/name conflict, scoped write, idempotent replay, wrong-scope
   403, size 413, layered 429, concurrency 503 and security-audit redaction.
10. Set `MCP_EXTERNAL_MUTATIONS_ENABLED=true`, redeploy, and start with a small
    invited cohort. Watch rejection rates and database load before posting to a
    public agent community.

## Disable / incident runbook

1. Set `MCP_EXTERNAL_MUTATIONS_ENABLED=false` in the production environment and
   redeploy immediately. Confirm an authenticated mutation returns 503 while
   `initialize`, `tools/list` and public read tools still succeed.
...
4. If the database guard RPCs are unavailable, leave the beta disabled. Do not
   switch to the legacy multi-request fallback in production.
...
- migration is exercised against PostgreSQL 16, including concurrent limits;
- production smokes cover one MCP 2025 client and one MCP 2026 OAuth client;
...
Vercel configuration change/redeploy; and rate-limit thresholds need tuning
from beta traffic. These are explicit reasons to run a bounded external beta,
not evidence that an unrestricted public launch is safe.
