...
- External mutations are fail-closed: they remain disabled unless explicitly
  enabled and a dedicated security pepper is configured.
...
| Kill-switch bypass | Unless `MCP_EXTERNAL_MUTATIONS_ENABLED=true` and a dedicated `MCP_SECURITY_PEPPER` is present, modern writes, legacy mutations and link/bootstrap paths fail closed; read/discovery remains available. |
...
- `503`: mutation kill switch, missing security guard configuration or an active concurrency lease, with `Retry-After`.
...
4. Re-apply the current `mcp_oauth_write_functions.sql` if the integration PR
   adds serialized standing-quota admission not yet deployed.
...
9. Smoke enrollment, scoped write, idempotent replay, wrong-scope 403, size 413,
   layered 429, concurrency 503 and security-audit redaction.
...
1. Set `MCP_EXTERNAL_MUTATIONS_ENABLED=false` in production and redeploy
   immediately. Confirm an authenticated mutation returns 503 while
