...
| `TELEGRAM_TOKEN`, `TELEGRAM_CHAT_ID` | `scripts/check_motus_raised.py`, `scripts/notify_agent_activity.py`, `scripts/notify_curator_verdicts.py`, `scripts/notify_source_proposals.py` (host cron, not a container) | Shared with the separate `mercator` project's own `@Vitruvyan_bot` — same bot, same chat, both live in `~/mercator/.env`. If lost here, copy back from there rather than creating a second bot. `TELEGRAM_TOKEN` is also mirrored on Vercel production (same value, `vercel env ls`) — the webhook route calls the Bot API directly and can't read this VPS `.env`. |
| `TELEGRAM_WEBHOOK_SECRET` | `app/api/telegram/webhook/route.ts` on Vercel only — kept here too so a redeploy of the webhook can be verified against the value actually registered with Telegram | The one thing standing between the public internet and `mcp_resolve_source_proposal` / a submission verdict: Telegram echoes it back on every webhook delivery in `X-Telegram-Bot-Api-Secret-Token`, and the route rejects anything that doesn't match. **Not recoverable if lost from both places** (this file and Vercel) — regenerate (`openssl rand -hex 32`), update both, then re-register with `setWebhook?url=...&secret_token=<new value>`. Safe to rotate any time; it authenticates Telegram to Terraveler, nothing depends on the old value once the webhook is re-registered. |
