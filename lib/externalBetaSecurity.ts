...
function protectedHash(value: string): string {
  const pepper = process.env.MCP_SECURITY_PEPPER ?? process.env.POSTGREST_SERVICE_KEY ?? "terraveler-beta";
  return createHmac("sha256", pepper).update(value).digest("hex").slice(0, 40);
}
...
export function requestSource(req: Request): RequestSource {
  ...
  return {
    ...
    sourceHash: protectedHash(`ip:${ip}`),
    networkHash: protectedHash(`network:${networkOf(ip)}`),
  };
}

export function mutationsEnabled(): boolean {
  return !/^(0|false|off|disabled)$/i.test(process.env.MCP_EXTERNAL_MUTATIONS_ENABLED ?? "true");
}
