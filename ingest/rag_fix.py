...
def run(setup):
    sql = setup + ("select round(similarity,3), left(title,40) from match_rag_docs('%s'::vector, 5, 'cortes-1519');" % lit)
    o = subprocess.run(["docker","exec","terraveler_postgres","psql","-U","terraveler","-d","terraveler","-c",sql],
        capture_output=True, text=True)
    return (o.stdout or o.stderr)
print("=== default (HNSW filter-after-ANN) ==="); print(run(""))
print("=== hnsw.iterative_scan=relaxed_order ==="); print(run("set hnsw.iterative_scan=relaxed_order; "))
print("=== exact (enable_indexscan=off) ==="); print(run("set enable_indexscan=off; set enable_bitmapscan=off; "))
